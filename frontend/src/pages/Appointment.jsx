...
const Appointment = () => {
  ...
  // NEW STRUCTURE:
  // docSlots = [
  //   { date: Date, slots: [{ datetime: Date, time: "10:00 AM" }, ...] },
  //   ...
  // ]
  ...
  const getAvailableSlots = useCallback(async () => {
    ...
    for (let i = 0; i < 7; i++) {
      // This is the date shown in the selector (fixed, stable)
      const dayDate = new Date(today)
      dayDate.setDate(today.getDate() + i)
      dayDate.setHours(0, 0, 0, 0)

      // This will be used for generating slots (changes time)
      ...
      // Store date + slots (even if slots are empty)
      setDocSlots((prev) => [
        ...prev,
        {
          date: dayDate,
  ...
  const bookAppointment = async () => {
    ...
    try {
      // Use the selected day date (stable)
      ...
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
            ...
            {/* OPTIONAL: show message when no slots */}
          ...
          <button
            ...
            className="bg-primary text-white text-sm font-light px-14 py-3 rounded-full my-6 hover:bg-primary/90 hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200"
          >
            Book an appointment
          </button>
        </div>
